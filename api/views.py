                ...
                if ProductMaster.objects.filter(barcode=barcode).exists():
                    ...
                    try:
                        PoTran(entry_no=PoHd.objects.get(pk=entry_no), line=line, product=product, packing=packing,
                               qty=qty, total_qty=total_qty, un_cost=un_cost, tot_cost=tot_cost,
                               pack_qty=pack_qty).save()
                        response['status'] = 200
                    ...
                    if PoTran.objects.filter(entry_no=entry).exists():
                        ...
                        for transaction in transactions:
                            ...
                            if cost['cost']['taxable'] == 1:
                                ...
                                levies = cost['cost']['tax_covid'] + cost['cost']['tax_nhis'] + cost['cost'][
                                    'tax_gfund']
        ...
        # cmms
            ...
            if header == 'carjob':
                ...
                if asset_count == 1:
                    ...
                    wo_count = \
                        cursor.execute(f"SELECT COUNT(*) from wo_hd where asset_code = '{asset_code}'").fetchone()[0]
                    old_wo_count = \
                        old_cursor.execute(f"SELECT COUNT(*) from wo_hd where asset_code = '{asset_code}'").fetchone()[
                            0]
                    result_list = []
                    old_q = f"SELECT top({limit}) created_by, wo.wo_no, wo.wreq_no, invoice_entry, wo.wo_date FROM wo_hd wo RIGHT JOIN w_request wr ON wr.wreq_no = wo.wreq_no WHERE wo.asset_code = '{asset_code}' order by wo.wo_date desc"
                    print(old_q)
                    if old_wo_count > 0:

                        old_wo = old_cursor.execute(
                            f"SELECT top({limit}) created_by, wo.wo_no, wo.wreq_no, invoice_entry, wo.wo_date FROM wo_hd wo RIGHT JOIN w_request wr ON wr.wreq_no = wo.wreq_no WHERE wo.asset_code = '{asset_code}' order by wo.wo_date desc"
                        )
