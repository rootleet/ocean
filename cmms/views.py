...
def stock(request):
    ...
    if StockCountHD.objects.filter(status=1).count() == 1:
        opened = True
    return render(request,'cmms/stock.html',context={'nav':True,'stocks':StockCountHD.objects.all(),'open':opened})
                    ...
                    if style == 'final_compare':
                        ...
                        if open_st == 1:
                            ...
                            for row in rows:
                                ...
                                item_q = cursor.execute(f"SELECT barcode from product_master where item_ref = '{itemRref}'")
                                ...
                                if StockCountTrans.objects.filter(item_ref=itemRref,stock_count_hd=stock_hd).exists():
                                    ...
                                    counted = StockCountTrans.objects.filter(item_ref=itemRref,stock_count_hd=stock_hd).aggregate(total=Sum('quantity'))['total']
                                obj = {
                                        'item_ref':itemRref,
                                        'barcode':barcode,
                                        'desription':name,
                                        ...
                                        'qty_diff':Decimal(counted) - Decimal(a_q)
                                }
                                if counted > 0:
                                    arr.append(obj)
                                else:
